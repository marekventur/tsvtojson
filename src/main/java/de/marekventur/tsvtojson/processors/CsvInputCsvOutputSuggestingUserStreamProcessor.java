...
public abstract class CsvInputCsvOutputSuggestingUserStreamProcessor<I extends CsvColumns, O extends CsvColumnsWithHeaderNames>
		extends CsvInputCsvOutputUserStreamProcessor<I, O> {

	protected static final String NULL_OUTPUT_VALUE = "";

	protected abstract void processRowWithSuggestion(Map<I, String> row,
			String[] suggestion, CSVWriter writer) throws IOException;

	private Map<O, I> suggestedColumMap = null;

	/**
	 * Returns an column suggestion lookup list or lazily creates it.
	 */
	...
	@Override
	protected final void processRow(Map<I, String> row, CSVWriter writer)
			throws IOException {
		String[] suggestion = new String[getOutputColumns().length];

		for (O outputColumn : getOutputColumns()) {
			String suggestedCell = row.get(getSuggestionIndexTable().get(
					outputColumn));
